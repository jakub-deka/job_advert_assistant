...
def count_tokens(string: str | list[str] | list[ChatMessage]) -> int:
    ...
    if isinstance(string, str):
        ...
        string
        and isinstance(string, list)
        and all(isinstance(s, ChatMessage) for s in string)
    ):
        string = [c.content for c in string]
    elif (
        string and isinstance(string, list) and all(isinstance(s, str) for s in string)
    ):
        pass
    else:
        raise ValueError(
            "Input must be a single string, list of strings or list of ChatMessage objects."
        )
...
class ContentProvider:
    ...
    def get_available_content(self):
        content = [p.stem for p in self.folder.glob("*.md")]
        return content

    def get_content(self, template: str):
        content = self.get_available_content()

        if template not in content:
            raise ValueError(f"Content piece called `{template}` is not available.")

        path = (self.folder / template).with_suffix(".md")
        with open(path, "r") as f:
            res = f.read()

        return res

    def write_content(self, template: str):
        st.markdown(self.get_content(template))
