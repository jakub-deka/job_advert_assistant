...
class RemoveLongStrings:
    ...
    def remove_string(self, string: str) -> bool:
        ...
        elif len(string) >= self.threshold: return True
        else: return False
    ...
    def run(self, strings: list[str]):
        res = []
        for s in strings:
            remaining_strings = [s for s in re.split(pattern=r" |\[|\]|\(|\)", string=s) if not self.remove_string(s)]
...
class Job:
    ...
    def construct_fetch_pipeline(self):
        ...
        fetch_pipeline.add_component("fetch", LinkContentFetcher(user_agents=[chrome_user_agent]))
        ...
        fetch_pipeline.add_component("remove_long_strings", RemoveLongStrings(threshold=31))
        fetch_pipeline.add_component("str_to_doc", StrToDocument())
        fetch_pipeline.add_component("clean", DocumentCleaner(remove_repeated_substrings=True))
    ...
    def run_pipeline(self, pipeline):
        try:
            success = True
            res = pipeline.run(data={"fetch":{"urls":[self.url]}})["clean"]["documents"][0].content
